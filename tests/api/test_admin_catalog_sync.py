...
def test_admin_catalog_sync_endpoint_persists_media(monkeypatch, tmp_path: Path) -> None:
    database_url = f"sqlite:///{tmp_path / 'catalog-sync-api.db'}"
    ...
    assert response.json() == {"root_path": "/Movies", "inserted": 1, "updated": 0}

    with Session(engine) as session:
        items = session.scalars(select(MediaItem)).all()

    assert len(items) == 1
    assert items[0].source_path == "/Movies/Movie.2024.mkv"
